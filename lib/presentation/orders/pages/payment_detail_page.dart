    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          BlocBuilder<CartItemBloc, CartItemState>(builder: (context, state) {
            final selectedPayment = state.maybeWhen(
                orElse: () => '',
                loaded: (_____, _, __, ___, ____, payment) {
                  return payment;
                });
            ...
            return ListView.separated(
              ...
              physics: const NeverScrollableScrollPhysics(),
              itemBuilder: (context, index) => PaymentMethod(
                isActive: selectedPayment == banksLimit[index].code,
                data: banksLimit[index],
                onTap: () {
                  context
                      .read<CartItemBloc>()
                      .add(CartItemEvent.AddPaymentMethod(
              ...
              BlocBuilder<CartItemBloc, CartItemState>(
                builder: (context, state) {
                  final totalOrder = state.maybeWhen(
                      orElse: () => 0,
                      loaded: (cartItems, __, ___, ____, _____, ______) =>
          ...
          Row(
            children: [
              ...
              BlocBuilder<CartItemBloc, CartItemState>(
                builder: (context, state) {
                  final totalOngkir = state.maybeWhen(
                    orElse: () => 0,
                    loaded: (_, __, ___, ____, shippingCost, _____) =>
                        shippingCost,
                  );
                  return Text(
                    totalOngkir.currencyFormatRp,
                    style: const TextStyle(
                      fontWeight: FontWeight.w600,
                    ),
                  );
          ...
          Row(
            children: [
              ...
              BlocBuilder<CartItemBloc, CartItemState>(
                builder: (context, state) {
                  final totalPay = state.maybeWhen(
                    orElse: () => 0,
                    loaded: (cartItems, _, __, ___, shippingCost, ____) =>
                        cartItems.fold<int>(
                          0,
                          (previousValue, element) =>
                              previousValue +
          ...
          BlocBuilder<CartItemBloc, CartItemState>(
            builder: (context, state) {
              final PaymentVA = state.maybeWhen(
                  orElse: () => '',
                  loaded: (_, __, ______, ___, ____, va) => va);

              //addressID
              final addressID = state.maybeWhen(
                  orElse: () => 0,
                  loaded: (_, addressID, ______, ___, ____, va) => addressID);

              //products
              final cartItems = state.maybeWhen(
                  orElse: () => [],
                  loaded: (products, __, _____, ___, ____, va) => products);

              //payment method
              final paymentMethod = state.maybeWhen(
                  orElse: () => '',
                  loaded: (_, __, paymentMethod, ___, ____, va) =>
                      paymentMethod);

              //shipping service
              final shippingService = state.maybeWhen(
                  orElse: () => '',
                  loaded: (_, __, ___, shippingService, ____, va) =>
                      shippingService);

              //shipping cost
              final shippingCost = state.maybeWhen(
                  orElse: () => 0,
                  loaded: (_, __, ___, ____, shippingCost, va) => shippingCost);
                ...
                child: BlocBuilder<OrderBloc, OrderState>(
                  builder: (context, state) {
                    return state.maybeWhen(
                        ...
                        return Button.filled(
                          ...
                          onPressed: () {
                            context.read<OrderBloc>().add(OrderEvent.doOrder(
                                  ...
                                  paymentVAName: PaymentVA,
                                  cartItems: cartItems as List<CartItem>,
                                ));
                            // context.pushNamed(
                            //   RouteConstants.paymentWaiting,
                            //   pathParameters: PathParameters().toMap(),
                      ...
                      // loaded: (orderResponseModel) => const Button.loading()
                    );
                  },
                ),
              );
